...
func NewKafkaOrderEventPublisher(brokers []string, topics map[string]string) entity.OrderEventPublisher {
	// If topics not provided, use defaults
	if topics == nil {
		topics = map[string]string{
			"orders":   "orders",
			"payments": "payments",
...
func (p *KafkaOrderEventPublisher) PublishOrderCreated(ctx context.Context, order *entity.Order) error {
	...
	return p.publishEvent(ctx, p.topics["orders"], order.ID, event)
}
...
func (p *KafkaOrderEventPublisher) PublishPaymentProcessed(ctx context.Context, order *entity.Order, payment *entity.Payment) error {
	...
	return p.publishEvent(ctx, p.topics["payments"], payment.ID, event)
}
...
func (p *KafkaOrderEventPublisher) PublishShippingUpdated(ctx context.Context, order *entity.Order, shipping *entity.Shipping) error {
	...
	return p.publishEvent(ctx, p.topics["shipping"], shipping.ID, event)
}
